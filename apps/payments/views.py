...
from payments.asaas.payments_dataclasses import (
    Billing,
    CreditCard,
...
class Step1(View):
    ...
    def post(self, request):
        ...
        return redirect(
            reverse('step2', kwargs={'invoice_id': response['id']})
        )
...
class Webhook(View):
    def post(self, request):
        ...
        external_reference = payload.get('payment', {}).get(
            'externalReference'
        )
        ...
        if ProcessedWebhook.objects.filter(
            Q(event_id=payload.get('id'))
            | (
                Q(invoice_id=invoice_id)
                & Q(event__in=['PAYMENT_RECEIVED', 'PAYMENT_CONFIRMED'])
            )
        ).exists():
